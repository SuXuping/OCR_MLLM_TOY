...
class OCR_MLLM_TOYLlamaConfig(LlamaConfig):
    model_type = "OCR_MLLM_TOY_llama"

class OCR_MLLM_TOYLlamaModel(LlamaModel):  ###LlamaModel只含有decoder的架构，OCR_MLLM_TOYMetaModel包含vision架构，所以此类是用于初始化整个vision和llm的decoder layer的模型
    config_class = OCR_MLLM_TOYLlamaConfig

    def __init__(self, config: LlamaConfig):  
        super(OCR_MLLM_TOYLlamaModel, self).__init__(config)
        if hasattr(config,"mm_vision_tower"):
            self.vision_tower = CLIPVisionTower("./ocr_mllm_toy/pretrain_weight/qwen_vit_448")
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):  ###将配置文件赋值为config文件，用于后面的保存
        ...
        if self.get_vision_tower() is None:
            ...
            vision_tower = CLIPVisionTower("./ocr_mllm_toy/pretrain_weight/qwen_vit_448")
...
class OCR_MLLM_TOYLlamaForCausalLM(LlamaForCausalLM):  
    config_class = OCR_MLLM_TOYLlamaConfig

    def __init__(self, config):
        super(OCR_MLLM_TOYLlamaForCausalLM, self).__init__(config)
        self.transformer = OCR_MLLM_TOYLlamaModel(config)
    ...
    def encode_images_high(self, images_high):
        image_features = self.get_model().get_vision_tower_high()(images_high)
        image_features = image_features.flatten(2).permute(0, 2, 1)
        image_features = self.get_model().mm_projector_high(image_features)
        return image_features
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and vision_tower is not None and images is not None and input_ids.shape[1] == 1:
                target_shape = past_key_values[-1][-1].shape[-2] + 1   ###qwen的key和value是(B,L,40,128)所以是-3,百川的是(B,40,L,128)所以是-2
...
AutoConfig.register("OCR_MLLM_TOY_llama", OCR_MLLM_TOYLlamaConfig)
AutoModelForCausalLM.register(OCR_MLLM_TOYLlamaConfig, OCR_MLLM_TOYLlamaForCausalLM)
