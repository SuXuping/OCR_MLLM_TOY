...
class BaichuanAttention(torch.nn.Module):
    ...
    def forward(
        ...
        # print(f"是否使用use_cache:{use_cache}")
        ...
        if xops is not None and self.training:
            ...
            # print(f'不使用 F.scaled_dot_product_attention')
...
class BaichuanModel(BaichuanPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
           ...
           #  print(f'input_ids 输入的形状：{input_ids.shape}')
            batch_size, seq_length = input_ids.shape
        elif inputs_embeds is not None:
            # print(f'inputs_embeds 输入的形状：{inputs_embeds.shape}')
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if hasattr(config, "quantization_config") and config.quantization_config['load_in_4bit']:
            ...
            if device_map is not None:
                dispatch_model(model, device_map=device_map) ###将不同设备上的模型，都放在device_map上面。Dispatches a model according to a given device map. Layers of the model might be spread across GPUs, offloaded on the CPU or even the disk.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print(f'input_ids的形状为：{input_ids.shape}')
