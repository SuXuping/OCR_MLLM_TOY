...
def _import_flash_attn():
    ...
    try:
        # print(f'成功>>>>>>>>：_import_flash_attn')
        from flash_attn.layers.rotary import apply_rotary_emb_func as __apply_rotary_emb_func
        apply_rotary_emb_func = __apply_rotary_emb_func
        # print(f'成功>>>>>>>>：__apply_rotary_emb_func')
    except ImportError:
        logger.warn(
...
class QWenAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            # print(f'成功使用FlashAttention')
            q, k, v = query, key, value
            attn_output = self.core_attention_flash(q, k, v, attention_mask=attention_mask)
        else:
            print(f'没有使用FlashAttention')
...
class QWenModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if (
            ...
            # print(f"成功使用flash attention")
            self.registered_causal_mask = None
        else:
            # print(f"没有使用flash attention")
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.use_flash_attn:
            _import_flash_attn()
        print(f'配置use_flash_attn：{config.use_flash_attn}')
    ...
    def forward(
        ...
        hidden_states = transformer_outputs[0]   ###([2, 779, 5120])

        lm_logits = self.lm_head(hidden_states)  ###([2, 779, 152064])

        loss = None
        if labels is not None:
            # logger.info(f'标签lable为：\n{labels}')
            labels = labels.to(lm_logits.device)
            shift_logits = lm_logits[..., :-1, :].contiguous()
            # logger.info(f'预测shift_logits为：\n{shift_logits}')
            shift_labels = labels[..., 1:].contiguous()
            # logger.info(f'标签shift_labels为：\n{shift_labels}')
