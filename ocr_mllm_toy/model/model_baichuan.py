...
from ocr_mllm_toy.model.language_model.baichuan2.modeling_baichuan import BaichuanForCausalLM,BaichuanModel
from ocr_mllm_toy.model.language_model.baichuan2.configuration_baichuan import BaichuanConfig

class OCR_MLLM_TOYBaichuanConfig(BaichuanConfig):
    model_type = "OCR_MLLM_TOY_Baichuan"

class OCR_MLLM_TOYBaichuanModel(BaichuanModel):  ###BaichuanModel只含有decoder的架构，OCR_MLLM_TOYMetaModel包含vision架构，所以此类是用于初始化整个vision和llm的decoder layer的模型
    config_class = OCR_MLLM_TOYBaichuanConfig

    def __init__(self, config: BaichuanConfig):  
        super(OCR_MLLM_TOYBaichuanModel, self).__init__(config)
...
class OCR_MLLM_TOYBaichuanForCausalLM(BaichuanForCausalLM):  
    config_class = OCR_MLLM_TOYBaichuanConfig

    def __init__(self, config):
        super(BaichuanForCausalLM, self).__init__(config)
        self.model = OCR_MLLM_TOYBaichuanModel(config)
...
AutoConfig.register("OCR_MLLM_TOY_Baichuan", OCR_MLLM_TOYBaichuanConfig)
AutoModelForCausalLM.register(OCR_MLLM_TOYBaichuanConfig, OCR_MLLM_TOYBaichuanForCausalLM)
