#!/bin/bash
deepspeed --include=localhost:2 --master_port 9913 ocr_mllm_toy/train/train_mem.py \
    --deepspeed ./scripts/zero2.json \
    --model_name_or_path ./checkpoints/llava-v1.5-qwen14b-pretrain-1m-ocr-448-1024-zero3 \
    --train_mode finetune_all \
    --version mpt \
    --data_path ./datasets/max_len_test.json \
    --image_folder "" \
    --vision_tower ./ocr_mllm_toy/pretrain_weight/qwen_vit_448 \
    --pretrain_mm_mlp_adapter ./checkpoints/llava-v1.5-13b-pretrain/mm_projector.bin \
    --mm_projector_type mlp2x_gelu \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end False \
    --mm_use_im_patch_token False \
    --image_aspect_ratio pad \
    --group_by_modality_length True \
    --bf16 True \
    --output_dir ./checkpoints/qwen_all_params_fintune \
    --num_train_epochs 1 \
    --per_device_train_batch_size 8 \
    --per_device_eval_batch_size 4 \
    --gradient_accumulation_steps 16 \
    --evaluation_strategy "no" \
    --save_strategy "steps" \
    --save_steps 500 \
    --save_total_limit 4 \
    --learning_rate 2e-5 \
    --weight_decay 0. \
    --warmup_ratio 0.03 \
    --lr_scheduler_type "cosine" \
    --logging_steps 1 \
    --tf32 True \
    --model_max_length 2048 \
    --gradient_checkpointing True \
    --dataloader_num_workers 4 \
    --lazy_preprocess True \
    --report_to wandb


# #!/bin/bash
# ###加入一些高质量的纯文本的数据集,中英各50万。使用lora，训练projection 和 baichuan2 lora模型
# deepspeed --include=localhost:1,2 --master_port 9913 llava/train/train_mem.py \
#     --deepspeed ./scripts/zero3.json \
#     --model_name_or_path  ./llava/baichuan_pretrain \
#     --version v1 \
#     --data_path /opt/MM_LLM/OCR-DATA/VQA/llava_1_5_stage2/llava_v1_5_mix665k_train.json \
#     --image_folder "" \
#     --vision_tower ./llava/clip-vit-large-patch14-336 \
#     --pretrain_mm_mlp_adapter ./checkpoints/llava-v1.5-baichuan-13b-5m-pretrain/checkpoint-6500/mm_projector.bin \
#     --mm_projector_type mlp2x_gelu \
#     --tune_mm_mlp_adapter True \
#     --mm_vision_select_layer -2 \
#     --mm_use_im_start_end False \
#     --mm_use_im_patch_token False \
#     --image_aspect_ratio pad \
#     --group_by_modality_length True \
#     --bf16 True \
#     --output_dir ./checkpoints/llava-v1.5-baichuan-13b-5m-finetune \
#     --num_train_epochs 1 \
#     --per_device_train_batch_size 8 \
#     --per_device_eval_batch_size 8 \
#     --gradient_accumulation_steps 1 \
#     --evaluation_strategy "no" \
#     --save_strategy "steps" \
#     --save_steps 500 \
#     --save_total_limit 4 \
#     --learning_rate 2e-5 \
#     --weight_decay 0. \
#     --warmup_ratio 0.03 \
#     --lr_scheduler_type "cosine" \
#     --logging_steps 1 \
#     --tf32 True \
#     --model_max_length 2048 \
#     --gradient_checkpointing True \
#     --dataloader_num_workers 4 \
#     --lazy_preprocess True \
#     --report_to wandb
